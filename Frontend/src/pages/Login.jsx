...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (tab === "signup") {
                ...
                toast.success("Account created successfully! Welcome to ResuMate.");
            } else {
    ...
    const handleGoogle = async () => {
        setLoading(true);
        await signIn("demo.google@resumate.ai", "googlepass");
    ...
    const clerkAppearance = {
        ...
        elements: {
            card: isDark 
                ? "bg-zinc-900/90 border border-zinc-800/80 shadow-2xl rounded-2xl" 
                : "bg-white border border-slate-200/80 shadow-xl rounded-2xl",
            headerTitle: isDark ? "text-white font-bold" : "text-slate-900 font-bold",
            headerSubtitle: isDark ? "text-zinc-400" : "text-slate-500",
            socialButtonsBlockButton: isDark 
                ? "bg-zinc-800/80 border-zinc-700/60 text-white hover:bg-zinc-700" 
                : "bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100",
            formFieldLabel: isDark ? "text-zinc-300" : "text-slate-700",
            formFieldInput: isDark 
                ? "bg-zinc-950 border-zinc-800 text-white focus:border-violet-500" 
                : "bg-slate-50 border-slate-200 text-slate-900 focus:border-violet-600",
            formButtonPrimary: "bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-xl shadow-md shadow-violet-500/20",
            footerActionLink: "text-violet-500 hover:text-violet-400 font-semibold",
